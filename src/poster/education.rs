...
pub(super) fn fill_education(&mut self, job: &JobRecord) -> BResult<()> {
    ...
    // 2) 已选值校验：已是目标则直接通过
    for sel in &["css:.ui-select-selected-value", "css:.ui-select-placeholder"] {
    ...
    // 3) 定位并点击“学历”触发器
    let trigger = ["css:.ui-select-selection", "css:.ui-select-inner", "css:.ui-select"]
        .iter()
        .find_map(|sel| edu_row.element(sel).ok().flatten())
        .ok_or_else(|| BossError::element("最低学历触发器"))?;
    trigger.run_js("this.scrollIntoView({block:'center', inline:'center'});").ok();
    ...
    for item in options {
        ...
        if clean == target_clean || clean.contains(&target_clean) || target_clean.contains(&clean) {
            item.click().ok();
            chosen = true;
            break;
        }
    ...
    if !chosen {
        let target_json = serde_json::to_string(&target)
            .map_err(BossError::map_config("学历序列化失败"))?;
        let js_script = r#"
            function() {
        ...
        if let Ok(value) = edu_row.run_js(&js_script) {
            if value.get("value").and_then(|v| v.as_bool()).unwrap_or(false) {
                chosen = true;
            }
        }
    }
